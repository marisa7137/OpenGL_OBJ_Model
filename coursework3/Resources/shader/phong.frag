...
uniform sampler2D tex_amb1;
uniform sampler2D tex_dif1;
uniform sampler2D tex_spec1;
uniform sampler2D tex_d1;
uniform sampler2D tex_bump1;
uniform sampler2D tex_amb2;
uniform sampler2D tex_dif2;
uniform sampler2D tex_spec2;
uniform sampler2D tex_d2;
uniform sampler2D tex_bump2;

vec3 CalDirectionalLight(DirectionalLight d_light, vec3 normal){
    vec3 ambient = d_light.ambient * material.ambient * vec3(texture(tex_amb1, TexCoords));
    // diffuse 
    vec3 lightDir = normalize(d_light.lightPos - FragPos);
    float diff = max(dot(normal, lightDir), 0.0);
    vec3 diffuse = d_light.diffuse * (diff * material.diffuse * vec3(texture(tex_dif1, TexCoords)));
...
vec3 CalcSpotLight(SpotLight s_light, vec3 normal)
{
    ...
    // combine results
    vec3 ambient = s_light.ambient * (material.ambient * vec3(texture(tex_amb1, TexCoords)));
    vec3 diffuse = s_light.diffuse * (diff * (material.diffuse * vec3(texture(tex_dif1, TexCoords))));
...
{    
    ...
    vec3 result = CalDirectionalLight(directionalLight, norm);
    if(isUsingSpotLight){
